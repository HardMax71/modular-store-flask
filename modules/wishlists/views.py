...
from .utils import get_product_selection_options, wishlist_exists, remove_from_wishlist, add_wishlist_item
...
def wishlist() -> ResponseValue:
    ...
    if wishlist_exists(user_id, product_id):
        remove_from_wishlist(user_id, product_id)
        flash(_("Product removed from your wishlist."), "success")
    else:
        add_wishlist_item(user_id, product_id, variant_options)
