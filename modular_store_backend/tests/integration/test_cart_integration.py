# /modular_store_backend/tests/integration/test_cart_integration.py
import json
import unittest
...
from modular_store_backend.modules.carts.utils import create_line_items_for_payment, \
    get_stripe_acc_for_customer_by_stripe_customer_id
from modular_store_backend.modules.db.models import Cart, Product, ShippingMethod, Address, Purchase, UserDiscount, \
    Discount, PurchaseItem
...
class TestCartIntegration(BaseTest):
    ...
    def setUp(self):
        ...
        self.full_test_product = Product(samplename='Test Product 2', price=1500, stock=121)
        ...
        self.session.add_all([self.product, self.shipping_method, self.address, self.full_test_product])
        self.session.commit()
    ...
    def test_full_purchase_flow(self):
        with self.app.test_request_context():
            ...
            # 1. Add item to cart
            response = self.client.post(url_for('carts.add_to_cart_route'), data={
                'product_id': self.full_test_product.id,
                'quantity': 2
            }, follow_redirects=True)
            self.assertEqual(response.status_code, 200)
            self.assertIn(b'Item added to cart', response.data)
            ...
            # 3. Simulate successful payment (Stripe webhook would normally handle this)
            response = self.client.get(url_for('carts.payment_success', session_id='cs_test123'), follow_redirects=True)
            self.assertEqual(response.status_code, 200)

            # 4. Verify purchase was successful
            self.assertIn(b"Test purchase completed", response.data)

            # 5. Check if the cart is empty after successful payment
            cart_items = self.session.query(Cart).filter_by(user_id=self.user.id).all()
            self.assertEqual(len(cart_items), 0)

            # 6. Verify that a purchase record was created
            # Test suite sets TESTING=true, so actual stripe isn't called
            purchase = self.session.query(Purchase).filter_by(user_id=self.user.id).first()
            self.assertIsNotNone(purchase)
            self.assertEqual(purchase.payment_method, 'test_payment')
            self.assertIn('test_', purchase.payment_id)  # Payment ID is generated by process_test_payment()

            # 7. Verify purchase items
            purchase_items = purchase.items
            self.assertEqual(len(purchase_items), 1)
            self.assertEqual(purchase_items[0].product_id, self.full_test_product.id)
            self.assertEqual(purchase_items[0].quantity, 2)

            # 8. Check if product stock was updated
            updated_product = self.session.get(Product, self.full_test_product.id)
            self.assertEqual(updated_product.stock, 119)  # Initial stock was 121, we bought 2
    ...
    @patch('modular_store_backend.modules.carts.views.stripe')
    @patch('modular_store_backend.modules.carts.utils.stripe')
    @patch('modular_store_backend.modules.carts.utils.save_purchase_history')
    ...
    @patch('modular_store_backend.modules.carts.views.stripe.checkout.Session.retrieve')
    @patch('modular_store_backend.modules.carts.views.process_successful_payment')
