...
from modular_store_backend.modules.db.models import Product, Category, UserPreference, Review, RecentlyViewedProduct
...
class TestRecommendations(BaseTest):
    ...
    def test_get_related_products(self):
        ...
        self.session.add_all([user_preference, review1, review2])
        self.session.commit()
    ...
    def test_get_recommended_products(self):
        ...
        review1 = Review(user_id=user.id, product_id=product1.id, rating=3)
        review2 = Review(user_id=user.id, product_id=product2.id, rating=5)
        self.session.add_all([user_preference1, user_preference2, review1, review2])
        self.session.commit()
